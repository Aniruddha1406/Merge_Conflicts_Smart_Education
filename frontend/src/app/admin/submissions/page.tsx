...
function dbToDisplay(row: any): DisplaySubmission {
  return {
    ...
    block: row.block,
    village: row.village,
    submittedBy: row.submitted_by_name || row.submittedBy,
    submittedAt: row.submitted_at || row.submittedAt,
    status: row.status,
    endorsements: row.endorsements,
    urgencyScore: row.urgency_score || row.urgencyScore,
    assignedInstitution: row.assigned_institution_name || row.assignedInstitution,
    fitScore: row.fit_score || row.fitScore,
    aiCategory: row.ai_category || row.aiCategory,
    aiSubcategory: row.ai_subcategory || row.aiSubcategory,
    aiPriority: row.ai_priority || row.aiPriority,
    aiTechnicalCore: row.ai_technical_core || row.aiTechnicalCore,
    aiAcademicField: row.ai_academic_field || row.aiAcademicField,
    aiTriage: row.ai_triage || row.aiTriage || 'INNOVATION_CHALLENGE', // Default mock triage
    aiConfidence: row.ai_confidence || row.aiConfidence || 0.95,
    aiKeywords: row.ai_keywords || row.aiKeywords,
    reporterType: row.reporter_type || row.reporterType,
  }
}
