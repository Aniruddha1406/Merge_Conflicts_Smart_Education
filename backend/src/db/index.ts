...
export const evidenceRepo = {
  ...
  insert(e: DbChallengeEvidence): void {
    try {
      getDb().prepare(`
        INSERT OR IGNORE INTO challenge_evidence (id, challenge_id, file_type, file_name, file_url, storage_path, file_size, uploaded_by_id, uploaded_at)
        VALUES (@id, @challenge_id, @file_type, @file_name, @file_url, @storage_path, @file_size, @uploaded_by_id, @uploaded_at)
      `).run(e)
    } catch (err) {
      console.error('[evidenceRepo.insert] failed:', err)
    }
  }
}
